...
const carpoolUrl = jsonData.carpoolUrl
...
function mdyToDMY(dateString) {
  // Split the date string by '/'
  const parts = dateString.split('/');

  // Re-arrange the parts to the 'dd/mm/yyyy' format
...
it('Submitting Carpool Microsoft Form', async () => {
  ...
  await labelElement.waitForExist({ timeout: 60000, timeoutMsg: 'Sign in was not found' })

  const { email, password } = { ...credentials }

  await carpoolPage.loginToMicrosoft(email, password)
  const carpoolTitle = await $('span=Car Pool Tracking Form')
  await expect(carpoolTitle).toExist()
  ...
  for (const data of carpoolData) {
    ...
    // Increment count after a successful submission
    formCount++

    console.table({
      "Submiited forms count ": formCount
    })

    // One form successfull, Thanks! label will be seen
    const submitAnotherFormLink = await $('a=Submit another response')
    await submitAnotherFormLink.waitForExist({
      timeoutMsg: `Last submitted carway data: date => ${date} carway => ${pickupLocation}`
    })
